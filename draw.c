...
double* cross_product(double x0, double y0, double z0, //P0
...
void draw_polygons( struct matrix *points, screen s, color c ) {
  ...
  for (point=0; point < points->lastcol-1; point+=3) {
    N = cross_product(points->m[0][point], points->m[1][point], points->m[2][point],
points->m[0][point+1], points->m[1][point+1], points->m[2][point+1],
points->m[0][point+2], points->m[1][point+2], points->m[2][point+2]);
    if (N[2] > 0){
...
void add_box( struct matrix *edges,
  ...
  //polygons
  add_polygon(edges, x, y, z, x1, y1, z, x1, y, z); 
  add_polygon(edges, x, y, z, x, y1, z, x1, y1, z);
...
void add_sphere( struct matrix * edges, 
  ...
  int index1, index2, index3, index4, lat, longt;
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      index2 = index1 + 1;
      index3 = (index2 + step) % points->lastcol;
      index4 = (index1 + step) % points->lastcol;

      add_polygon(edges, points->m[0][index1], points->m[1][index1], points->m[2][index1],
		points->m[0][index2], points->m[1][index2], points->m[2][index2],
		points->m[0][index3], points->m[1][index3], points->m[2][index3]);
      add_polygon(edges, points->m[0][index1], points->m[1][index1], points->m[2][index1],
		points->m[0][index3], points->m[1][index3], points->m[2][index3],
		points->m[0][index4], points->m[1][index4], points->m[2][index4]);
    }
  }
  free_matrix(points);
}
